...
const BASE_URL = import.meta.env.VITE_BASE_URL || "https://mhdigitaledge.com/api/user";
...
export const categoryService = {
  ...
  getAllCategories: async (): Promise<ICategory[]> => {
    try {
      const response = await apiClient.get<CategoriesResponse>('/fetch/category');
  ...
  getCategoryById: async (id: string): Promise<ICategory> => {
    try {
      const response = await apiClient.get<ApiResponse<ICategory>>(`/fetch/category/${id}`);
...
export const servicesService = {
  ...
  getServicesByCategory: async (categoryId: string): Promise<IService[]> => {
    try {
      const response = await apiClient.get<ServicesResponse>(`/fetch/category/${categoryId}`);
  ...
  getServiceById: async (id: string): Promise<IService> => {
    try {
      const response = await apiClient.get<SingleServiceResponse>(`/fetch/services/${id}`);
  ...
  getAllServices: async (): Promise<IService[]> => {
    try {
      const response = await apiClient.get<ServicesResponse>('/fetch/services');
      return extractServices(response.data);
    } catch (error) {
...
export const teamService = {
  ...
  getAllTeams: async (): Promise<ITeamMember[]> => {
    try {
      const response = await apiClient.get<{ teamMembers: ITeamMember[]; message: string }>('/fetch/teams');
...
export const faqService = {
  ...
  getAllFaqs: async (): Promise<IFaq[]> => {
    try {
      const response = await apiClient.get<{ faqs: IFaq[]; message: string }>('/fetch/faqs');
...
export const projectService = {
  ...
  getAllProjects: async (): Promise<IProject[]> => {
    try {
      const response = await apiClient.get<{ projects: IProject[]; message: string }>('/fetch/projects');
  ...
  getProjectById: async (id: string): Promise<IProject> => {
    try {
      const response = await apiClient.get<{ project: IProject; message: string }>(`/fetch/projects/${id}`);
...
export const testimonialService = {
  ...
  getAllTestimonials: async (): Promise<ITestimonial[]> => {
    try {
      const response = await apiClient.get<ITestimonial[]>('/fetch/testimonail');
...
export const contactService = {
  ...
  sendContactForm: async (contactData: IContactForm): Promise<ContactResponse> => {
    try {
